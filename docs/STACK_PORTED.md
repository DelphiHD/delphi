...
The cost spine for HD Reports runs through three categories: **memory architecture** (mnemo-cortex, OpenBrain, memory-palace), **token compression** (lean-geck), and **the build harness** (superpowers, everything-claude-code, claude-behave). These are the only repos that touch the per-report dollar number directly. Everything else is supporting infrastructure (transcription, scraping, design, security, viz) or reference material.
...
The single most load-bearing repo for Phase 4 is **lean-geck** (fork of `yvgude/lean-ctx`). Its README claims "up to 99% reduction" in token consumption via shell-output compression and context-server tools, with monthly real-world savings of "$30-100+." That's the lever that makes the under-30-cents-per-report target plausible. It's an MCP server — drops in next to whatever else `invoke-llm` calls.
...
- **Friction.** **License is FSL-1.1-MIT, not plain MIT.** Functional Source License has a non-compete clause that converts to MIT after 2 years — needs a read by Tennyson before HD Reports goes paid. The "OB1" branding in the parent repo is a different project than Nate B. Jones' public "OpenBrain" content; verify the fork is the version we want.
...
- **HD Reports touch-points.** **The Phase 4 cost lever.** Drop in front of `invoke-llm`'s Anthropic call. README claims "up to 99%" reduction with examples like "session total 88% savings (~89,800 tokens to ~10,620)." Even if real-world numbers are half that, it's the difference between hitting the 30-cent target and missing it.
- **Maturity.** Production per README. Distributed via four package managers.
- **Friction.** It's a Rust binary and shell-hook stack — meant for an interactive coding environment, not a Supabase Edge Function. **The architectural fit needs a real look.** In Edge Function context (Deno runtime), we likely can't run the binary; we'd port the compression patterns we want. Possible Phase 4 sub-task: extract the prompt-side compression rules and reimplement as a TypeScript module callable from `invoke-llm`. Don't expect a drop-in.
...
- **License audit on FSL-1.1-MIT (OpenBrain) and unspecified license (safe-chain-aikido-security).** HD Reports is going paid; confirm both are usable in a commercial context before committing them to Phase 4 / CI.
...
- **lean-geck integration model.** Cannot run a Rust binary inside a Supabase Edge Function (Deno). Decide: (a) port compression rules to TypeScript and inline in `invoke-llm`, (b) run lean-geck as a sidecar HTTP service that `invoke-llm` calls, or (c) run lean-geck only on the dev side and accept smaller production savings. **(a) is probably the right answer**, but it's real work, not "drop in."
...
1. **`supabase/functions/invoke-llm` — prompt caching + cost ceiling.** Implement directly per the master plan (cache_control on system prompt, 80-cent abort). No port required — this is plain Anthropic SDK code.
2. **`supabase/functions/invoke-llm` — token compression layer.** Port the relevant subset of `lean-geck`'s context-compression rules from Rust to TypeScript and apply pre-send. Target: input-token reduction of 50%+ on the report-generation call before we even hit cache reads. Treat the 99% upstream claim as ceiling, not floor.
