...
- **Never assert a connection, channel, or gate the chart does not have.** Describe only the channels, gates, and center-to-center connections this chart actually has. Never say two centers connect (e.g. "the Sacral connects to the Throat") unless they share a defined island in this chart. For a Manifesting Generator, the definition is a defined Sacral PLUS any one of the four motors (Sacral, Heart, Solar Plexus, Root) connected to the Throat through defined channels, however long the path — the connecting motor NEED NOT be the Sacral. When the chart's structural facts state which motor reaches the Throat, use that; if the Sacral does not reach the Throat, never describe a Sacral-to-Throat path. Do not invent a gate the chart lacks to make a connection work.
...
TYPE MECHANICS FOR THIS CHART (deterministic — ground the Your Type section in this; do not improvise the architecture, and never repeat the words "type justification" or "data pass" to the reader):
