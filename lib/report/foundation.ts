...
**Never present a universal mechanic as this chart's own.** Every cross without exception is built from the four gates the Sun and Earth occupy on the Personality and Design sides, so "yours is built from the four gates of your two Sun-Earth axes" implies that other people's crosses are built from something else. Either say it as the universal it is, or leave it out. The same goes for the seventy percent figure and for "every chart has an Incarnation Cross": those describe the term, not this reader. Open with what is actually true of THIS cross, which is its geometry, its quarter and its theme.
