...
- **Never say how far away a date is.** No "four years away", "next year", "a decade from now", "in about eighteen months", no arithmetic against the present at all. You are not told today's date and cannot know it: asked to work it out you will answer from where your training data ends, which is how a 2027 date became "approximately four years away" in a report written in 2026. The report is evergreen and will be re-read for decades. State the date and the window; let the reader do the subtraction on the day they are reading.
...
- **Never assert a connection, channel, or gate the chart does not have.** Describe only the channels, gates, and center-to-center connections this chart actually has. Never say two centers connect (e.g. "the Sacral connects to the Throat") unless they share a defined island in this chart. The deterministic structural facts below state exactly which centers this chart connects and how — describe those and only those, and never invent a gate the chart lacks to make a connection work.
...
- **A fixing state gets its own sentence.** When a placement carries an exaltation or a detriment, say it in a short sentence of its own. Never hang it off a longer sentence as a subordinate clause. Write: "The design Uranus sits at the second line in detriment. That describes an unconscious emotional support that can overstep its moment." Not: "...while the design Uranus at the second line, in detriment, describes an unconscious emotional support that can overstep its moment." Across this product's reports, sentences carrying a fixing state run 37% longer than the rest and hold more than twice the technical vocabulary, and that density is where readers stop reading. Keep any sentence carrying a fixing state under 25 words. Say the placement, stop, then say what it does.
...
**Explain what an Incarnation Cross is, and explain it as a universal.** Most readers do not know what a cross is, so the section opens by telling them: every cross is built from the four gates the Sun and Earth occupy on the Personality and Design sides, and those four carry most of the conditioning that reaches a design. Keep that. Write it as a general fact about crosses. NEVER write it in the second person: "yours is built from the four gates of your two Sun-Earth axes" tells the reader that other people's crosses are built from something else, which is false and is the one thing this rule exists to stop. "Every cross is built from..." is right; "your cross is built from..." is not. Then move to what is actually particular to this chart: its geometry, its quarter, its theme.
...
The architecture you describe in Your Type must match the chart's actual definition. Do NOT claim "your Sacral motor is connected directly to the Throat center" unless a channel between the Sacral and the Throat actually exists in this specific chart. Read the chart's actual channels and centers and describe THOSE — not the generic architecture of a different Type.

TYPE MECHANICS FOR THIS CHART — INTERNAL QA NOTES (deterministic). These are engine notes for YOU, to keep the architecture correct. They are NOT content for the reader and must never surface in the report. Obey them silently. Do NOT quote or paraphrase them; do NOT name, define, or contrast against any other Type ("rather than a Manifesting Generator," "unlike a Projector"); and do NOT copy any "do NOT / pattern-match / trap" wording into the prose. Describe ONLY the reader's own design, in the reader's own terms, using the centers and channels this chart actually has:
${dataPass.typeJustification}
...
- Describe the reader's OWN Type on its own terms, from the centers and connections this chart actually has. Do NOT name, define, or contrast against any other Type, and do NOT use framing that only means something as a comparison to another Type (e.g. "the manifestation path," "unlike a Generator"). The reader is new to Human Design; another Type's mechanics mean nothing to them and do not belong in their report.
${dataPass.type === "Manifesting Generator" ? `- Name which motor centers reach the Throat in THIS chart and what each brings to how the reader's energy comes out: Sacral = generative response, Solar Plexus = emotional depth, Heart = willpower, Root = pressure and drive. Say what that specific combination means for how this person's energy reaches expression. If the Sacral itself does not reach the Throat, describe where it does connect (e.g. paired with the G center) as part of this design, stated plainly and never as something missing.\n` : ``}
